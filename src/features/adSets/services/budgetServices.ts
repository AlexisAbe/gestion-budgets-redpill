...
export async function updateAdSetActualBudgetService(
  ...
  try {
    ...
    const { data: existingAdSet, error: fetchError } = await supabase
      .from('ad_sets')
    ...
    const actualBudgets = existingAdSet.actual_budgets || {};
    ...
    toast.error(`Erreur lors de la mise à jour du budget: ${error instanceof Error ? error.message : 'Erreur inconnue'}`);
    return false;
  }
}
